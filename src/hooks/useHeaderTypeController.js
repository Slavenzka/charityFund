...
function useHeaderTypeController () {
  const [className, setClassName] = useState([css.header])
  ...
  const removeClassName = useCallback(className => {
    setClassName(prevState => {
      ...
      if (targetIndex >= 0) {
        return [].concat(state.slice(0, targetIndex), state.slice(targetIndex + 1))
      }
  ...
  const addClassName = useCallback(className => {
    setClassName(prevState => {
      const state = prevState.slice()
